...
export default function Dashboard() {
  ...
  const baixoEstoque = medicamentos.filter(m => m.estoque_atual <= m.estoque_minimo)
  ...
  // Consumo mensal — agrupa saídas por mês
  const consumoMensal = Object.values(
    movimentacoes
      .filter(m => m.tipo === 'saida')
      .reduce((acc, m) => {
        const mes = MESES[new Date(m.created_at).getMonth()]
        acc[mes] = acc[mes] || { mes, unidades: 0 }
        acc[mes].unidades += Number(m.quantidade)
        return acc
      }, {})
  )

  // Top medicamentos — por saídas
  const topMeds = Object.values(
    movimentacoes
      .filter(m => m.tipo === 'saida')
      .reduce((acc, m) => {
